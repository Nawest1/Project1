...
@app.route('/view-holdings', methods=['GET'])
def view_portfolio():
    form = ViewPortfolio()
    portfolios = Portfolio.query.all()
    for portfolio in portfolios:
        form.portfolio.choices.append((portfolio.id, portfolio.name))
    portfolio_id = form.portfolio.data
    stockholdings = Stock.query.filter_by(portfolio_id=form.portfolio.data)
    portolioholding = stockholdings
    return render_template("viewportfolio.html", form=form, portfolio_id=portfolio_id, portolioholding=portolioholding)
